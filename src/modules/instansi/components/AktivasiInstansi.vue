          ...
          <v-stepper-items>
            <v-stepper-content step="1">
              <v-card text class="rounded-lg">
                ...
                <v-card-title primary-title class="justify-space-around mb-0 pb-0">
                  <h3 class="tittle primary--text d-block text-center">Kode Token</h3>
                </v-card-title>
                <v-card-text>
                  <v-form data-vv-scope="form-cekToken" lazy-validation>
                    <v-row column wrap>
                      <v-col cols="12" class="justify-space-around text-center"
                        >Silakan melakukan aktivasi akun Instansi Anda dengan menginputkan Kode Token yang Anda
                        dapat</v-col
                      >
                      <v-col cols="12" class="my-3 py-0">
                        <v-text-field
                          ...
                          v-validate="'required'"
                          :error-is_ssos="errors.collect('form-cekToken.token')"
                        ></v-text-field>
                      </v-col>
            ...
            <v-stepper-content step="2">
              <v-card text class="rounded-lg mb-0 pb-0">
                ...
                <v-card-title primary-title class="justify-space-around mb-0 pb-0">
                  <h3 class="tittle primary--text d-block text-center">Informasi Akun</h3>
                </v-card-title>
                <v-card-text class="mb-0 pb-0">
                  <v-form @submit.prevent data-vv-scope="form-sandi" lazy-validation>
                    <v-row align="center" justify="center">
                      <v-col cols="12" class="justify-space-around text-center"
                        >Langkah selanjutnya silakan inputkan Data Akun yang nantinya akan Anda gunakan untuk Login
                        sebagai Admin Sekolah Anda</v-col
                      >
                      <v-col cols="12" sm="12" md="12" lg="12" xl="12">
                        ...
                        ></v-text-field>
                        <p class="orange--text">{{ passpor ? passpor.pesan : '' }}</p>

                        <v-text-field
                          v-if="passpor && passpor.status == 9"
                          ref="password"
                          outlined
                          dense
                          persistent-hint
                ...
                <v-card-actions class="pt-0 mt-0">
                  <v-btn
                    ...
                    color="secondary"
                    @click="passpor && passpor.status == 2 ? setPassword() : setPassword()"
                    style=" webkit-box-shadow: 0px 2px 7px 5px #D3D3D3;"
                    >{{ passpor && passpor.status == 2 ? 'Aktivasi' : 'Aktivasi' }}</v-btn
                  >
                </v-card-actions>
              </v-card>
            </v-stepper-content>
            <v-stepper-content step="3">
              <v-card text class="rounded-lg">
                ...
                <v-card-title primary-title class="justify-space-around mb-0 pb-0">
                  <h3 class="tittle primary--text d-block text-center">Tahap Akhir Aktivasi</h3>
                </v-card-title>
                <v-card-text>
                  <v-form data-vv-scope="form-aktivasi" lazy-validation>
                    <v-row column wrap>
                      <v-col cols="12" class="justify-space-around text-center">
                        <p class="my-0 py-0"
                          >Silahkan klik tombol aktivasi yang telah dikirimkan ke email anda untuk mengaktifkan akun
                          instansi pada layanan SIAP Belajar</p
                        >
                      </v-col>
                      <v-col v-show="false" cols="12" class="my-3 py-0">
                        <v-text-field
                          label="Kode Aktivasi"
                          v-model="kodeAktivasi"
                          hint="Silakan masukkan kode kode aktivasi Anda*"
                          outlined
                          dense
                          persistent-hint
                          ...
                          v-validate="'required'"
                          :error-is_ssos="errors.collect('form-aktivasi.kode_aktivasi')"
                        ></v-text-field>
                      </v-col>
                      <v-col v-show="false" cols="12" class="justify-space-around text-center pt-0 mt-0">
                        <v-btn
                          :loading="loading"
                          medium
                          block
                ...
                <v-card-text class="black--text">
                  <div v-if="user">
                    <p class="mt-3 text-center">
                      Anda telah berhasil melakukan aktivasi akun. Silakan Login dengan surel di atas melalui tombol
                      berikut</p
                    >
                    <p
                      ><v-btn
...
export default {
  ...
  mounted() {
    this.checkQuery()
    // this.kodeAktivasi = this.$route.params.kode_aktivasi ? this.$route.params.kode_aktivasi : null
  },
  methods: {
    checkQuery() {
      if (this.$route.query.kode_aktivasi) {
        this.st = 3
        this.kodeAktivasi = this.$route.query.kode_aktivasi
        // clean query
    ...
    cekToken() {
      this.$validator
        ...
        .then(() => {
          return this.$store
            .dispatch('instansi/cekToken', this.token)
            .then((res) => {
              this.user = res
              this.st = 2
              return res
    ...
    proses() {
      ...
      this.$validator
        ...
        .then(() => {
          return this.$store
            .dispatch('instansi/aktivasiToken', { email: this.user.email, token: this.token })
            .then((res) => {
              this.st = 4
              this.dialog = true
              return res
    ...
    aktivasi() {
      ...
      this.$validator
        ...
        .then(() => {
          return this.$store
            .dispatch('user/aktivasiAkunInstansi', { kodeAktivasi: this.kodeAktivasi })
            .then((res) => {
              this.st = 4
              this.dialog = true
              return res
    ...
    setPassword() {
      this.$validator
        ...
        .then(() => {
          return this.$store
            .dispatch('user/setPasswordInstansi', {
